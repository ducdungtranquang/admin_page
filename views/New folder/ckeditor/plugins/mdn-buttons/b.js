	...
	e.extend(h.prototype, {
		...
		setAttribute: (function() {
			...
			if (c && (b.ie7Compat || b.ie6Compat)) return function(j, k) {
				...
				else if (j == 'contenteditable') i.call(l, 'contentEditable', k);
		...
		function o(u) {
			...
			return function(y) {
				...
				if (y.type == 3) {
					...
					if (!n[y.getName()])
						if (!c && y.is('br') && !v && !(u && y.getNext())) v = true;
						else return false;
				return true;
			};
		};
		...
		d.range.prototype = {
			...
			optimize: function() {
				...
				var u = w.startContainer,
					v = w.startOffset;
				if (u.type != 1)
					if (!v) w.setStartBefore(u);
					else if (v >= u.getLength()) w.setStartAfter(u);
				u = w.endContainer;
				v = w.endOffset;
				if (u.type != 1)
					if (!v) w.setEndBefore(u);
				...
				switch (u) {
					case 1:
						...
						if (G.type == 3) {
							...
							if (G)
								if (!(B = G.getPrevious())) A = G.getParent();
						} else {
			...
			checkReadOnly: (function() {
				function u(v, w) {
					while (v) {
						if (v.type == 1)
							if (v.getAttribute('contentEditable') == 'false' && !v.data('cke-editable')) return 0;
			...
			moveToElementEditablePosition: function(u, v) {
				...
				while (u) {
					if (u.type == 3) {
						if (v && this.checkEndOfBlock() && w.test(u.getText())) this.moveToPosition(u, 3);
						else this.moveToPosition(u, v ? 4 : 3);
						y = 1;
						break;
					}
					if (u.type == 1)
	...
	(function() {
		...
		function L(N, O, P) {
			...
			var R = O.createBookmark(),
				...
				T = S.lastElement.getAscendant('li', 1),
				U = S.block.getBogus();
			U && U.remove();
			var V = Q.getLast();
			if (V && V.type == 1 && V.is('br')) V.remove();
			var W = O.startContainer.getChild(O.startOffset);
			if (W) Q.insertBefore(W);
			else O.startContainer.append(Q);
			var X = new d.elementPath(P.startContainer),
				Y = P.startContainer.getAscendant('li', 1);
			if (Y) {
				var Z = M(Y);
				if (Z)
					if (T.contains(Y)) {
			...
			init: function(N) {
				...
				N.on('key', function(Q) {
					...
					if (N.mode == 'wysiwyg' && R in {
						...
						var U = R == 8,
							V = N.document.getBody(),
							W = new d.walker(T.clone());
	...
	(function() {
		...
		a.dialog = function(Q, R) {
			...
			if (Q._.removeDialogTabs && (W = Q._.removeDialogTabs[R]))
				for (X = 0; X < W.length; X++) S.removeContents(W[X]);
			...
			this.on('ok', function(aq) {
				ah(function(ar) {
					if (ar.validate) {
						var as = ar.validate(this),
							at = typeof as == 'string' || as === false;
						if (at) {
							aq.data.hide = false;
							aq.stop();
						}
			...
			function ai() {
				...
				aq.sort(function(at, au) {
					if (at.tabIndex != au.tabIndex) return au.tabIndex - at.tabIndex;
					else return at.focusIndex - au.focusIndex;
				});
				var ar = aq.length;
			...
			function aj(aq) {
				...
				var at = (as + aq + ar.length) % ar.length,
					au = at;
			...
			var ao = [],
				ap = a.dialog._.uiElementBuilders.hbox.build(this, {
					type: 'hbox',
		...
		a.dialog.prototype = {
			...
			foreach: function(Q) {
				...
				for (var R in T._.contents)
					for (var S in T._.contents[R]) Q.call(T, T._.contents[R][S]);
				return T;
			},
			reset: (function() {
				var Q = function(R) {
			...
			addPage: function(Q) {
				...
				var W = b,
					...
					Y = h.createFromHtml(['<a class="cke_dialog_tab"', ac._.pageCount > 0 ? ' cke_last' : 'cke_first', S, !!Q.hidden ? ' style="display:none"' : '', ' id="', X, '"', W.gecko && W.version >= 10900 && !W.hc ? '' : ' href="javascript:void(0)"', ' tabIndex="-1"', ' hidefocus="true"', ' role="tab">', Q.label, '</a>'].join(''));
			...
			if (!X) {
				...
				if (b.ie6Compat) {
					var Z = b.isCustomDomain(),
						aa = "<html><body style=\\'background-color:" + T + ";\\'></body></html>";
		...
		(function() {
			var Q = /^([a]|[^a])+$/,
				...
				S = /^\d*(?:\.\d+)?$/,
				T = /^(((\d*(\.\d+))|(\d*))(px|\%)?)?$/,
				U = /^(((\d*(\.\d+))|(\d*))(px|em|ex|in|cm|mm|pt|pc|\%)?)?$/i,
				V = /^(\s*[\w-]+\s*:\s*[^:;]+(?:;|$))*$/;
			a.VALIDATE_OR = 1;
			a.VALIDATE_AND = 2;
			a.dialog.validate = {
				functions: function() {
					var W = arguments;
					return function() {
						var X = this && this.getValue ? this.getValue() : W[0],
							Y = undefined,
							Z = 2,
							aa = [],
							ab;
						for (ab = 0; ab < W.length; ab++) {
							if (typeof W[ab] == 'function') aa.push(W[ab]);
							else break;
						}
					...
					if (A) {
						...
						for (var D = 0; D < C.length; D++) {
							...
							var H = E.startContainer,
								...
								L = E.clone();
							if (H && H.type == 3)
								if (J >= H.getLength()) L.setStartAfter(H);
								else L.setStartBefore(H);
							if (I && I.type == 3)
								if (!K) L.setEndBefore(I);
